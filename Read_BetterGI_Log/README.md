...
- 修改其他訊息
  - 隠去 地圖识别錯誤，取上次点位
  - 隠去 未识别到具体路径，打开地图计算中心点
  - 新增 多次尝试未恢复，尝试最小化后激活窗口！
### 20251116
- 增加錯誤&其他訊息
  - 用户取消，终止运行
  - 检测到空月祝福界面，自动点击
### 20251102
  - alt+I拾取物以拾取物拾取順序方式顯示
### 20251031
- 修正H鍵筛选任務時，O鍵排序拾取物效率后退出H鍵筛选任務后、所有拾取物行顯示異常
- 增加錯誤&其他訊息
  - 未识别到具体路径，取上次点位
  - 未识别到具体路径，打开地图计算中心点
  - 鋤地一條龍1.10.0
...
- 增加錯誤&其他訊息
  - 队伍切换失败
- 新增O鍵：排序指定拾取物的拾取物效率功能
  - 建議先用 H 鍵篩選要排序的配置組、然后再用 H 排序"指定拾取物"的拾取效率
    - 顯示為 N秒/1个
### 20251027
- 修正任務過短沒被顯示的錯誤
### 20251022
- 調整手機版CSS
- 修复了文件名版本号被错误截断的问题
- 不顯示"XX任务已到禁止执行时段，将跳过"的任务
- 新增配置组导航中 排除配置组名
  - 筛选函数支持多个关键词

### 20251020
- 增加錯誤&其他訊息
  - 简易策略脚本要求的角色不存在！队伍中需要存在下面角色中的一个或多个：".*"
...
### 20251016
- 增加錯誤&其他訊息
  - 任务启动失败：当前存在正在运行中的独立任务，请不要重复执行任务！

### 20251008
- 使任务名称只顯示文件名（不含路径和扩展名）
- 修正在隱藏0錯誤任務時，進入筛选模式會顯示所有任務問題
